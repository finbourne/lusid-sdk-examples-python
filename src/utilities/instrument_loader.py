...
class InstrumentLoader:
    __InstrumentSpec = namedtuple("__InstrumentSpec", ["Figi", "Name"])
    ...
    async def load_instruments(self):
        instruments_to_create = {
            i.Figi: models.InstrumentDefinition(
                name=i.Name,
                identifiers={"Figi": models.InstrumentIdValue(value=i.Figi)},
            )
            for i in self.__instruments
        }
        response = await self.instruments_api.upsert_instruments(
            request_body=instruments_to_create
        )
