...
async def main():
    ...
    api_factory = lusid.extensions.ApiClientFactory(
        config_loaders=(
            ...
            EnvironmentVariablesConfigurationLoader(),
            # then from secrets file.
            SecretsFileConfigurationLoader(secrets_file_path),
        )
    )
    async with api_factory:
        ...
        portfolio = await tx_portfolios_api.create_portfolio(
        ...
        figis_to_create = {
            "BBG000C6K6G9": models.InstrumentDefinition(
        ...
        tx1 = models.TransactionRequest(
            ...
            instrument_identifiers={
                "Instrument/default/LusidInstrumentId": name_to_luid[
                    "VODAFONE GROUP PLC"
                ]
            },
            ...
            total_consideration=models.CurrencyAndAmount(
                amount=103 * 100, currency="GBP"
            ),
            source="Broker",
        )

        await tx_portfolios_api.upsert_transactions(
            scope=scope, code=portfolio_code, transaction_request=[tx1]
        )
